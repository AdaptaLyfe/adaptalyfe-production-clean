...
**How to apply:** Keep controller reads inside the dialog state, guard post-`await` UI updates with `mounted`, disable duplicate submission while the action is active, and let a success listener perform the single `Navigator.pop`. Confirmation actions should also check `mounted` before dispatching after an awaited dialog.

Backend delete actions should constrain the row by both resource ID and authenticated user ID, and API clients should replace non-user-facing HTML, JSON, or framework payloads with a safe status-based message.