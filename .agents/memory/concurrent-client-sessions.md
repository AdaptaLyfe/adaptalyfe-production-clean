...
**How to apply:** Issue bearer tokens only for the native-client request path; validate bearer identity in request-specific auth context rather than persisting it into cookie session state. Startup/auth-page guards must validate the shared `/api/user` endpoint instead of assuming the native token key is present, because wrapper/browser sessions may use the HttpOnly cookie path. Native login/register responses may be minimal, so hydrate the canonical `/api/user` record before entering the authenticated app. Refresh that record on app resume without replacing the authenticated state during the request. Cookie and bearer logout must invalidate only the credential used by that client. Keep username equality exact and preserve shared subscription entitlement across both clients.